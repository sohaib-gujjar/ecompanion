...
export class init1632145121706 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`e-companion\`.\`message\` (\`id\` char(36) NOT NULL, \`createdAt\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` datetime NULL, \`text\` varchar(255) NOT NULL, \`messageId\` char(36) NULL, \`fileId\` char(36) NULL, \`userId\` char(36) NULL, UNIQUE INDEX \`REL_ce2da017b61bdb9fd01415a618\` (\`fileId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`e-companion\`.\`message\` ADD CONSTRAINT \`FK_b664c8ae63d634326ce5896cecc\` FOREIGN KEY (\`messageId\`) REFERENCES \`e-companion\`.\`message\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`e-companion\`.\`message\` DROP FOREIGN KEY \`FK_b664c8ae63d634326ce5896cecc\``);
