...
export class init1632145121706 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`e-companion\`.\`file\` (\`id\` char(36) NOT NULL, \`createdAt\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` datetime NULL, \`originalname\` varchar(255) NULL, \`filename\` varchar(255) NOT NULL, \`mimetype\` varchar(255) NULL, \`path\` varchar(255) NOT NULL, \`destination\` varchar(255) NULL, \`ext\` varchar(255) NULL, \`size\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`e-companion\`.\`message\` (\`id\` char(36) NOT NULL, \`createdAt\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updatedAt\` datetime NULL, \`text\` text NOT NULL, \`fileId\` char(36) NULL, \`userId\` char(36) NOT NULL, \`toUserId\` char(36) NULL, \`workspaceMessageId\` char(36) NULL, \`teamsMessageId\` char(36) NULL, UNIQUE INDEX \`REL_ce2da017b61bdb9fd01415a618\` (\`fileId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`e-companion\`.\`teams_users_user\` ADD CONSTRAINT \`FK_f675843e42c3220d3c34bd81e1d\` FOREIGN KEY (\`userId\`) REFERENCES \`e-companion\`.\`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
